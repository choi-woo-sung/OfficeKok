	...
	<div class="table-header">
		...
		<form id="searchForm">
			...
			<div class="input-unread pull-right m-t-10">
				...
				<input type="checkbox" id="checkIsRead" name="isImportant" class="checkbox" value="1" title="중요" />
				&nbsp;&nbsp;&nbsp;&nbsp;
			</div>
			
			<div class="input-unread pull-right m-t-10">
				<label><i class="fas fa-envelope text-blue"></i>&nbsp;unRead&nbsp;</label>
				<input type="checkbox" id="checkIsRead" name="isRead" class="checkbox" value="1" title="읽지 않은 목록" />
				&nbsp;&nbsp;&nbsp;&nbsp;
			</div>
			...
			<table id="noteTable" class="table table-striped table-bordered">
				<thead style="width:100%; text-align:center;">
					<tr>
						...
						<th><button style="border:none; background:none;" type="button" id="isRead" title="isRead" />
						...
						<c:when test="${not empty dataList }" >
							<c:forEach var="note" items="${dataList }">
								<tr>
									...
									<td><i id="check-circle" class="fa fa-check-circle text-grey mt-1"></i></td>
									<td><!-- <i class="fa fa-file mt-2"></i> --></td>
									<td>
										<c:if test="${'N' eq note.noteread }">
											<i class="fas fa-envelope text-blue mt-1"></i>
										</c:if>
										<c:if test="${'Y' eq note.noteread }">
											<i class="fas fa-envelope-open text-grey mt-1"></i>
										</c:if>
									</td>
									<td>${note.filtercatenm }</td>
									<c:url value="/note/receiveView/${note.noteno }" var="noteReceiveViewURL"></c:url>
									<td>
										<a class="text-black-darker" href="${noteReceiveViewURL }">${note.notesubject }</a>
									</td>
									<td>${note.sender }</td>
									<td>${note.notedate }</td>
								</tr>
