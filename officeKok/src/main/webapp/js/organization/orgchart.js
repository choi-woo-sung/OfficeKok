                ...
                if (k) {
                    ...
                    if (a.left || a.right || a.up || a.down) {
                        ...
                        b.startMove(a, function(t) {
                            _[4] = n + t.x, _[5] = o + t.y, k.x = l - t.xWithoutScale, k.y = s - t.yWithoutScale, e.x = h - t.xWithoutScale, e.y = d - t.yWithoutScale, E.setAttribute("transform", "matrix(" + _.toString() + ")")
                        })
                    } else {
                        ...
                        var m = (e.x - k.x) / A,
                            C = (e.y - k.y) / A;
                        if (_[4] = S + m, _[5] = I + C, !y._dragEventFired && (Math.abs(e.x - k.x) > OrgChart.FIRE_DRAG_NOT_CLICK_IF_MOVE || Math.abs(e.y - k.y) > OrgChart.FIRE_DRAG_NOT_CLICK_IF_MOVE)) !1 === OrgChart.events.publish("drag", [b, c]) && M(), y._dragEventFired = !0;
                        E.setAttribute("transform", "matrix(" + _.toString() + ")")
                    }
                }
            },
            M = function(t) {
...
OrgChart.prototype._mouseDownHandler = function(t, e, r) {
    ...
    if (e == OrgChart.action.init && a.expand && a.expand.nodes && null == n)
        ...
        if (r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS)
            for (d = 0; d < b.childrenIds.length; d++) {
                (k = l[b.childrenIds[d]]).collapsed = !1;
                for (var w = 0; w < k.childrenIds.length; w++) {
                    l[k.childrenIds[w]].collapsed = !0
                }
            }
        if (r.options.parentState === OrgChart.COLLAPSE_PARENT_SUB_CHILDREN_EXCEPT_CLICKED)
            if (y = l[b.pid])
                for (d = 0; d < y.childrenIds.length; d++) {
                    var k;
                    if ((k = l[y.childrenIds[d]]) != b) {
                        k.collapsed = !1;
                        for (w = 0; w < k.childrenIds.length; w++) {
                            l[k.childrenIds[w]].collapsed = !0
                        }
                    }
                }
    ...
    var _ = [],
        ...
        A = {},
        E = [];
    for (d = 0; d < h.length; d++) OrgChart.server._iterate(h[d], h[d], l, 0, _, S, I, A, "", t, a, e, v, r, E, n);
    ...
    for (d = 0; d < E.length; d++) {
        ...
        for (w = 0; w < f.children.length; w++) {
            (U = f.children[w]).isAssistant || (-1 != U.tags.indexOf("right-partner") ? (U.isPartner = 1, U.children = [], M.push(U.id)) : -1 != U.tags.indexOf("left-partner") ? (U.isPartner = 2, U.children = [], N.push(U.id)) : -1 == U.tags.indexOf("partner") || B % 2 ? -1 != U.tags.indexOf("partner") && B % 2 ? (U.isPartner = 2, U.children = [], N.push(U.id), B++) : null != U.ppid ? (T[U.ppid] || (T[U.ppid] = []), T[U.ppid].push(U.id)) : L.push(U.id) : (U.isPartner = 1, U.children = [], M.push(U.id), B++))
        }
        f.children = [];
        ...
        for (w = N.length - 1; 0 <= w; w--)
            if (T[N[w]])
                for (var R = 0; R < T[N[w]].length; R++) f.children.push(l[T[N[w]][R]]), P++, -1 == H.indexOf(N[w]) && H.push(N[w]);
        ...
        for (w = 0; w < M.length; w++)
            if (T[M[w]])
                for (R = 0; R < T[M[w]].length; R++) f.children.push(l[T[M[w]][R]]), F++, -1 == z.indexOf(M[w]) && z.push(M[w]);
    ...
    for (d = 0; d < I.length; d++) {
        if ((at = t[(f = l[I[d]]).lcn ? f.lcn : "base"]).layout == OrgChart.normal || !A[f.pid])
            for (w = 0; w < f.subLevels; w++) {
                var j = new OrgChart.node(f.id + "_sub_level_index_" + w, f.pid, [], "subLevel");
                if (OrgChart.server._initDinamicNode(j, f.lcn), y = f.parent) {
                    var D = y.children.indexOf(f); - 1 < D && (y.children.splice(D, 1), y.children.splice(D, 0, j)), j.children.push(f), j.parent = y, f.parent = j, l[j.id] = j
                }
            }
    }
    for (var Y in S) {
        (y = l[Y]).hasAssistants = !0;
        j = new OrgChart.node(y.id + "_split_0", y.id, ["assistant"], "split");
        OrgChart.server._initDinamicNode(j, y.lcn, !0), l[j.id] = j;
        var X = [];
        for (w = y.children.length - 1; 0 <= w; w--) {
            (U = y.children[w]).isAssistant ? (U.parent = null, y.children.splice(w, 1), X.splice(0, 0, U.id)) : U.isPartner || ((U.parent = j).children.unshift(U), y.children.splice(w, 1))
        }
        if (X.length % 2) {
    ...
    if ($)
        for (var Y in A) {
            if ((at = t[(y = l[Y]).lcn ? y.lcn : "base"]).layout == OrgChart.mixed) {
                var et = A[Y];
                for (d = et.length - 1; 0 <= d; d--) {
                    if (y = (U = l[et[d]]).parent, U.layout = OrgChart.mixed, 0 < d) {
                        for (w = y.children.length - 1; 0 <= w; w--) U.id == y.children[w].id && y.children.splice(w, 1);
                        var rt = l[et[d - 1]];
    ...
    if (it)
        for (var Y in A) {
            var at;
            if ((at = t[(y = l[A[Y][0]].parent).lcn ? y.lcn : "base"]).layout == OrgChart.tree || at.layout == OrgChart.treeRightOffset || at.layout == OrgChart.treeLeftOffset) {
                j = new OrgChart.node(y.id + "_split_0", y.id, [], "split");
                OrgChart.server._initDinamicNode(j, y.lcn), (l[j.id] = j).layout = OrgChart.tree;
                var nt = [];
                for (d = A[Y].length - 1; 0 <= d; d--) {
                    for (U = l[A[Y][d]], w = 0; w < y.children.length; w++) y.children[w].id == U.id && y.children.splice(w, 1);
    ...
    if (C.top = l - i.padding, C.left = o - i.padding, C.bottom = n + i.padding - e / v, C.right = a + i.padding - t / v, C.maxX = a, C.minX = o, C.maxY = n, C.minY = l, null != s || f || i.align != OrgChart.CENTER) {
        if (null == s && !f && i.align == OrgChart.ORIENTATION) {
            b = Math.ceil(t / v), w = Math.ceil(e / v), k = 0, _ = 0;
            switch (i.orientation) {
                case OrgChart.orientation.top:
                    k = u.x - b / 2 + OrgChart.server._getNodeWidth(u, i) / 2, _ = -i.padding;
                    break;
                case OrgChart.orientation.bottom:
                    k = u.x - b / 2 + OrgChart.server._getNodeWidth(u, i) / 2, _ = i.padding - w;
                    break;
                case OrgChart.orientation.left:
                    k = -i.padding, _ = u.y - w / 2 + OrgChart.server._getNodeWidth(u, i) / 2;
                    break;
                case OrgChart.orientation.right:
                    k = i.padding - b, _ = u.y - w / 2 + OrgChart.server._getNodeWidth(u, i) / 2;
                    break;
                case OrgChart.orientation.top_left:
                    k = -i.padding, _ = -i.padding;
                    break;
                case OrgChart.orientation.right_top:
                    k = i.padding - b, _ = -i.padding;
                    break;
                case OrgChart.orientation.left_top:
                    k = -i.padding, _ = -i.padding;
                    break;
                case OrgChart.orientation.bottom_left:
                    k = -i.padding, _ = i.padding - w
            }
            s = [k, _, b, w], i.sticky && (s[0] < C.left && s[0] < C.right && (s[0] = C.left > C.right ? C.right : C.left), s[0] > C.right && s[0] > C.left && (s[0] = C.left > C.right ? C.left : C.right), s[1] < C.top && s[1] < C.bottom && (s[1] = C.top > C.bottom ? C.bottom : C.top), s[1] > C.bottom && s[1] > C.top && (s[1] = C.top > C.bottom ? C.top : C.bottom))
        }
    } else {
        var b = Math.ceil(t / v),
            w = Math.ceil(e / v),
            k = 0,
            _ = 0;
        if (b - 2 * i.padding >= a - o) switch (k = (a + o) / 2 - b / 2, i.orientation) {
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
                k = (o - a) / 2 - b / 2
        } else switch (k = u.x - b / 2 + OrgChart.server._getNodeWidth(u, i) / 2, i.orientation) {
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
                (k = -(b / 2 - (o - a) / 2)) < i.padding - b && (k = i.padding - b);
                break;
            case OrgChart.orientation.left:
            case OrgChart.orientation.bottom_left:
            case OrgChart.orientation.top_left:
            case OrgChart.orientation.left_top:
                (k = -(b / 2 - (a - o) / 2)) > -i.padding && (k = -i.padding)
        }
        if (w - 2 * i.padding >= n - l) switch (_ = (n + l) / 2 - w / 2, i.orientation) {
            case OrgChart.orientation.bottom:
            case OrgChart.orientation.bottom_left:
                _ = (l - n) / 2 - w / 2
        } else switch ((_ = -(w / 2 - (n - l) / 2)) > -i.padding && (_ = -i.padding), i.orientation) {
            case OrgChart.orientation.bottom:
            case OrgChart.orientation.bottom_left:
                (_ = -(w / 2 - (l - n) / 2)) < i.padding - w && (_ = i.padding - w);
                break;
            case OrgChart.orientation.left:
            case OrgChart.orientation.right:
                _ = u.y - w / 2 + OrgChart.server._getNodeWidth(u, i) / 2
        }
        s = [k, _, b, w]
    }
    ...
    link: function(t, e, r, i, a) {
        ...
        for (f = 0; f < t.children.length; f++) {
            ...
            var k = "";
            for (var _ in e.config.linkBinding) {
                var S = e.config.linkBinding[_],
                    I = e._get(m.id);
                if (I) {
                    var A = I[S];
                    null != A && null != A && null != l[_] && (k += l[_].replace("{val}", A))
                }
            }
            "" != k && (k = OrgChart.linkFieldsOpenTag.replace("{x}", y.x).replace("{y}", y.y).replace("{rotate}", 0) + k + OrgChart.grCloseTag, s.push(w.html)), s.push(OrgChart.grCloseTag)
        }
        return s.join("")
    },
    ...
    function t(t, e, r) {
        ...
        if (A += '<g c-link-from="{from}" c-link-to="{to}">'.replace("{from}", t.id).replace("{to}", e.id) + s.link.replaceAll("{d}", b), M.label) {
            ...
            A += s.label.replace("{x}", x.x).replace("{y}", x.y + _).replace("{val}", M.label)
        }
    ...
    return e
}, OrgChart.miniMap.colors = ["#E0E0E0", "#BDBDBD", "#9E9E9E", "#757575"], OrgChart.miniMap.selectorBackgroundColor = "white", OrgChart.miniMap.backgroundColor = "white", OrgChart.events.on("redraw", function(t, e) {
    if (t.config.state) {
        var i = [],
            a = [];
        ! function t(e) {
