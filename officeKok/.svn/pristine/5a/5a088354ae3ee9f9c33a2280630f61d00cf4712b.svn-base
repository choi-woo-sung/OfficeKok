	...
	<tbody class="text-center">
		<c:choose>
			<c:when test="${not empty paging.data}">
				<c:forEach items="${paging.data}" var="reject" varStatus="status">
					<tr>
						<td class="f-s-600 text-inverse">${reject.rn}</td>
						<td>
							<a class="f-s-600 text-inverse" href="${pageContext.request.contextPath}/approval/work/apprReject/${reject.apprdocno}">
								${reject.apprdocsubject}
							</a>
						</td>
						<td>${reject.deptnm}</td>
						<td>${reject.empnm}</td>
						<td>${reject.apprdocdate}</td>
						<td>${reject.apprhisdate}</td>
						<td>
							<c:if test="${reject.apprdocstate eq '4'}">
								반려
							</c:if>
						</td>
					</tr>
