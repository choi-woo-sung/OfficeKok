...
$(function () {
  ...
  recognition.onstart = function () {
    ...
    const defaultMsg = '전 하이콕 입니다.';
    textToSpeech(defaultMsg);
  };
  ...
  function noteread() {
		$.ajax({
			url : "/officeKok/main/note",
			method : "get",
			dataType : "json",
			success : function(resp) {
				console.log(resp.res);
				textToSpeech(resp.res);
			},
...
// console.log('interimTranscript', interimTranscript);
  ...
  function fireCommand(string) {
	  console.log(string);
   /* if (string.indexOf('메일')>=0 || string.indexOf('매일')>=0  || string.indexOf('내일')>=0  ) {
    	location.href="/officeKok/mail/private" ;
    } else*/ if (string.indexOf('받은')>=0 || string.indexOf('바다')>=0 || string.indexOf('꼭지')>=0 || string.indexOf('복지')>=0 
    		||string.indexOf('가든')>=0 ) {

 location.href="/officeKok/note/receive" ;
    } else if (string.indexOf('근태')>=0 || string.indexOf('변태')>=0 ) {
    	location.href="/officeKok/attendance/oneday" ;
    } else if (string.indexOf('일정')>=0 || string.indexOf('내일')>=0 ) {
    	location.href="/officeKok/cal" ;
    } else if (string.endsWith('읽어 줘')) {
        noteread();
        isRecognizing = false;
        recognition.stop();
        return false;
    } /*else if (string.indexOf('조직도')) {
    	location.href="/officeKok/groupChart" ;
    }*/ else if (string.endsWith('홈') ||string.endsWith('봄')  || string.endsWith('폼') ) {
    	location.href="/officeKok/main" ;
  ...
  /**
	 * 문자를 음성으로 읽어 줍니다. 지원: 크롬, 사파리, 오페라, 엣지
	 */
